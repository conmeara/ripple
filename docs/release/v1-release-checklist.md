...
| Do not accept proxy signals as completion | Command output, packaged app evidence, manual QA notes, artifact inspection | Substantially satisfied locally. Automated gates, package/resource smoke, local MP4/MOV/WebM export smoke, packaged UI MP4 export, packaged analytics opt-in/off smoke, and credentialed provider smoke passed; official notarized CI release, update N-to-N+1 smoke, and remaining human QA still gate stable. |
| Ship the release as `v0.19` | `package.json`, release workflow input, GitHub release tag/name | Package metadata and packaged `Info.plist` are `0.19.0`; final signed/notarized release evidence still needs the official GitHub Actions path for `v0.19`. |
...
| Package build | `bun run package` or platform package command | Packaged app contains expected resources and identity | Passed 2026-05-05 for local `--dir`; export browser staged into `Resources/browser`; notarization skipped locally. |
...
- Run the official GitHub Actions release workflow for `v0.19` and verify
  signed, notarized, stapled artifacts with `codesign`, `spctl`, and `stapler`.
