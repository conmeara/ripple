...
releases. Update checks are non-blocking and never prevent local project
creation/opening, preview, comments, revisions, or export.
...
- [ ] Implement Milestone 2: replace generic CDN/R2 release assumptions with
  GitHub Releases publishing and update metadata.
...
- Observation: App Store Connect Team API key access is confirmed for
  ...
  `WRSCQ49MY6` with the downloaded local key file at
  `/Users/conmeara/Downloads/AuthKey_WRSCQ49MY6.p8`. The private key contents
  were not copied into the plan.
...
- Decision: Use GitHub Releases for Phase 18 unless implementation discovery
  proves a separate generic host is materially safer.
  Rationale: The user prefers GitHub Releases, and it is simpler for an open
  source/personal project than maintaining a separate update CDN.
  Date/Author: 2026-05-04 / Codex.
...
`autoDownload = false`, configures a generic feed URL from
...
workflow should be manually triggered, build Ripple, sign/notarize macOS
artifacts, create or update a draft GitHub Release, attach artifacts, and attach
the selected update metadata. Use the built-in `GITHUB_TOKEN` with explicit
`contents: write` workflow permission for release writes unless implementation
discovery proves a maintainer PAT is required. Local publishing can exist only
as a maintainer fallback using ignored `.env` values. Do not read a GitHub token
from app runtime code.

Fourth, adapt update metadata. Either configure `electron-builder` to publish
GitHub release artifacts and update metadata directly, or adapt the current
manifest script so it uploads/attaches the correct channel metadata to GitHub
Releases. Remove assumptions about old CDN/R2 upload destinations.

Fifth, wire signing and notarization. For macOS, collect
`CSC_LINK`, `CSC_KEY_PASSWORD`, `APPLE_IDENTITY`, `APPLE_TEAM_ID`, and
App Store Connect Team API key credentials. Store them locally in ignored
`.env` for maintainer fallback packaging and as GitHub Actions secrets for CI.
Use Apple ID/app-specific password only as a fallback if API-key notarization is
blocked. Validate that the installed app can update from one signed/notarized
build to a newer signed/notarized build.

Sixth, refine user-facing update UX. Keep downloads user-initiated, keep beta
optional, show meaningful release/version state, and ensure failure states are
recoverable. Rename inherited "Agents"/"Beta" wording where needed so the
settings read as Ripple app updates rather than developer-tool beta features.
...
3. Add a GitHub Actions release workflow with manual `workflow_dispatch`,
   explicit `permissions: contents: write`, draft-release publishing, and
   stable/beta inputs.
...
7. Decide whether to switch from generic `setFeedURL` to electron-builder's
   GitHub provider, or keep generic metadata hosted as GitHub Release assets.
8. Update release scripts and the CI workflow to build, sign, notarize, create
   draft GitHub Releases, attach artifacts, and attach `latest`/`beta` metadata.
9. Update `MAIN_VITE_RIPPLE_UPDATE_URL` or provider configuration so packaged
   builds discover GitHub-hosted update metadata.
10. Update settings copy and release notes links to Ripple/GitHub Releases.
11. Add or update tests for update channel preference and renderer update state.
12. Build two macOS versions in GitHub Actions, install the older one, and verify
   it updates to the newer signed/notarized one.
13. Update this ExecPlan with exact artifacts, tags, workflow runs, and validation
   evidence.
...
- `bun run build`
- `bun run package:mac`
- `bun run dist:manifest` if the manifest path remains in use

Manual packaged QA:

- Install a signed/notarized macOS build at version N with stable channel.
  Expected: it checks the GitHub Releases stable feed and either reports no
  update or offers version N+1 if published.
...
- Click Update when an update is available. Expected: download starts only after
  the click, progress is shown inside Ripple, the user can restart/install from
  Ripple, and no browser/GitHub/manual asset selection is required.
...
- macOS update install is validated with signed/notarized artifacts.
...
- Release notes: open `https://github.com/conmeara/ripple/releases` unless the
  release repo changes.
...
Known local notarization values:

- `CSC_LINK=.secrets/signing/ripple-developer-id.p12`
- `CSC_KEY_PASSWORD` is set in the ignored local `.env` and stored at
  `.secrets/signing/p12-password.txt`
...
- `APPLE_API_KEY=/Users/conmeara/Downloads/AuthKey_WRSCQ49MY6.p8`
...
- Confirm the first public release version/tag. Initial releases stay as drafts
  until signed/notarized update validation passes.
...
- Decide whether current generic metadata on GitHub Release assets is enough,
  or whether to switch to electron-builder's GitHub provider configuration.
