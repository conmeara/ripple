...
> = {
  ...
  "project-setup": `Create hidden Ripple setup config for this project.

Your task:
1. Analyze the project to understand what a hidden revision copy needs
2. Create the file .ripple/worktree.json with setup commands

The goal is to reproduce the same working state as the primary project in the hidden revision copy.
...
- Use $RIPPLE_PROJECT_PATH to reference the primary project path
- Don't include build steps unless absolutely necessary for the project to work

Example output for .ripple/worktree.json:
{
  "setup-worktree": [
    "bun install",
    "cp $RIPPLE_PROJECT_PATH/.env .env",
    "cp $RIPPLE_PROJECT_PATH/.env.local .env.local"
  ]
}

Now analyze this project and create .ripple/worktree.json with the appropriate setup commands.`,
}
...
): type is "review" | "pr-comments" | "release-notes" | "security-review" | "commit" | "project-setup" {
