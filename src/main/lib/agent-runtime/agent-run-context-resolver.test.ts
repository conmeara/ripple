...
describe("agent run context resolver", () => {
  test("separates app policy, project notes, and app-managed skill roots", async () => {
    ...
    try {
      ...
      expect(context.appPolicy).toContain("ripple frame-sheet --range 0s..8s --samples 8 --columns 4 --json")
      expect(context.appPolicy).toContain("ripple snapshot --at current --json")
      expect(context.appPolicy).toContain("ripple snapshot --at 1.25s --json")
  ...
  test("keeps Codex and Claude Ripple visual-context skill bodies in sync", async () => {
    ...
    expect(codexSkill).toContain("the best visual tools for agents are the app-managed `ripple")
    ...
    expect(codexSkill).toContain("ripple frame-sheet --range 0s..8s --samples 8 --columns 4 --json")
