...
describe("Claude runtime capabilities source contract", () => {
  ...
  test("auto-allows only the app-managed visual commands for Claude visual context", async () => {
    const source = await readFile(
    ...
    expect(source).toContain("\"Bash(ripple snapshot *)\"")
    expect(source).not.toContain(`"Bash(ripple ${"context"})"`)
    expect(source).not.toContain(`"Bash(ripple ${"context"} *)"`)
    expect(source).not.toContain("\"Bash(ripple sheet)\"")
    expect(source).not.toContain("\"Bash(ripple sheet *)\"")
