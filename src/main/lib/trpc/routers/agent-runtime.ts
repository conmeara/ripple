...
import { createAgentRunReplayObservable } from "../../agent-runtime/run-replay-subscription"
...
export const agentRuntimeRouter = router({
  ...
  resumeRun: publicProcedure
    ...
    .subscription(({ input }) => {
      return createAgentRunReplayObservable(input.runId, {
        getRun: getAgentRun,
        listEvents: listAgentRunEvents,
        subscribe: subscribeToAgentRunEvents,
      })
    }),
