...
const CMDS = {
  ...
  lint: { one: 'pre-render cut-safety check for the whole manifest', what: 'The cheap gate before rendering: every scene’s endpoints are checked from cached perception. Exit 1 when a blocking finding remains.', how: 'Walks edit.json scenes, resolves each source’s cached index, runs the same endpoint checks as candidates, and reports missing or degraded perception evidence. It never analyzes or writes.', io: 'Reads edit.json + cached indexes → JSON verdict with per-scene findings; exit code is the contract.' },
